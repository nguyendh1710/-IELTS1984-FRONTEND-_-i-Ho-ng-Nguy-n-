...
import {Container,Logo,Input,DropArea,AnswerBox,ConfettiWrapper,InstructionsContainer,Overlay,BackgroundImage,SubmitButton} from './QuizzTest.style'
...
const { data:quizzArr = [], error, isLoading } = useQuery({
...
if (error) {
  ...
  const handleStop = (e, item) => {
    ...
    if (isInDropZone && item.word === quizzData.question.blanks[1].correctAnswer) {
  ...
  const renderParagraph = () => {
    ...
    return (
      <>
        ...
        <span dangerouslySetInnerHTML={{ __html: '. You should drag the word <span style="color: red;">green</span>' }} />
      </>
    );
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <Container>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', marginTop: '160px', marginLeft: '275px' }}>
          <p style={{ textAlign: 'center' }}>
            <span style={{ fontWeight: 'bold' }}>Question: </span>
            {renderParagraph()}
          </p>

          <div style={{ width: '200px', height:'200px', position: 'absolute', right: '740px', top: '200px' }}>
            <Animation />
          </div>

          <div style={{ display: 'flex', flexDirection:'column', alignItems: 'center', marginTop: '50px',marginRight: '60px' }}>
            <Typography sx={{ fontWeight: 'bold', mr: 1 }}>Answers List</Typography>
            <div style={{ width: '100px', marginLeft: '20px', display: 'flex', flexDirection: 'column', marginTop: '10px' }}>
        ...
        {word.word}
          ...
          {submitted && (
            <div style={{ marginTop: '20px', marginRight: '50px' }}>
              {isCorrect ? (
                <p style={{ color: 'green' }}>Chính xác! Bạn đã trả lời đúng.</p>
              ) : (
        ...
        <div style={{
          ...
          <InstructionsContainer>
            <Box sx={{ width: '100%', textAlign: 'center', mb: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 'bold', fontSize: '23px' }}>
                Instructions
              </Typography>
            </Box>

            {/* Step 1 */}
            <Box sx={{ ml: 1, mb: 1, display: 'flex', alignItems: 'center', flexWrap: 'nowrap' }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', textAlign: 'left', fontSize: '15px', mr: 1 }}>
                Step 1:<span style={{ fontWeight: '200' }}> Fill in input box number 1.</span>
              </Typography>
            ...
            <Box sx={{ ml: 1, mb: 1, display: 'flex', alignItems: 'center', flexWrap: 'nowrap' }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', textAlign: 'left', fontSize: '15px', mr: 1 }}>
                Step 2: <span style={{ fontWeight: '200' }}> Drag the answer, and drop it into input box number 2.</span>
              </Typography>
            </Box>
            ...
            <Box sx={{ ml: 1, mb: 1, display: 'flex', alignItems: 'center', flexWrap: 'nowrap' }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', textAlign: 'left', fontSize: '15px', mr: 1 }}>
                Step 3: <span style={{ fontWeight: '200' }}> Click the submit button to check the results.</span>
              </Typography>
            </Box>
          </InstructionsContainer>
          ...
          <SubmitButton
            ...
            sx={{ fontWeight: 'bold', fontSize: '22px', textAlign: 'left', mr: 3 }}
          >
            Submit
          </SubmitButton>
        </div>
